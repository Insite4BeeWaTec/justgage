...
JustGage = function(config) {
    ...
    // Create min label and set value
    ...
    obj.minLabelObject.attr({
        ...
        "font-family": "Arial",
        "fill": obj.config.labelFontColor,
        "fill-opacity": (obj.config.hideMinMin || obj.config.donut) ? "0" : "1"
    });
    setDy(obj.minLabelObject, obj.params.minFontSize, obj.params.minY);
    this.setNewMin(obj.config.min ? obj.config.min : 0)

    // Create max label and set value
    ...
    this.setNewMax(obj.config.max ? obj.config.max : 0)
...
JustGage.prototype.setNewMin = function(min) {
    ...
    if (min !== null && obj.config.min !== min && !isNan(parseFloat(min))) {
        obj.config.min = min;

        var newText;
        if (obj.config.formatNumber) {
            newText = formatNumber(obj.config.min);
        } else {
            newText = obj.config.min.toString();
        }
        obj.minLabelObject.attr({ "text": newText });
        setDy(obj.minLabelObject, obj.params.minFontSize, obj.params.minY);
    }
}
...
JustGage.prototype.setNewMax = function(max) {
    ...
    // Check if value is valid and different
    if (max !== null && obj.config.max !== max && !isNan(parseFloat(max))) {
        obj.config.max = max;

        var newText;
        if (obj.config.formatNumber) {
            newText = formatNumber(obj.config.max);
        } else {
            newText = obj.config.max.toString();
        }
        obj.maxLabelObject.attr({ "text": newText });
        setDy(obj.maxLabelObject, obj.params.maxFontSize, obj.params.maxY);
    }
}
